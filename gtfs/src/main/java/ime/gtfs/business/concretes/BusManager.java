...
public class BusManager implements BusService {
	...
	for (Bus bus : buses) {
			...
			System.out.println(bus.getAgency().getAgencyName()+" - "+"Vehicle type: "+bus.getRoute().getRouteType()+" | " + " Bus Route: " +bus.getRoute().getRouteShortName()+" " + bus.getRoute().getRouteLongName());
			...
			for (StopTime stopTime : stpTimes) {
				System.out.println("Stop Time Id: "+ stopTime.getStopTimeId()+" Arrival Time: " + stopTime.getArrivalTime()+" Departure Time: " +stopTime.getDepartureTime() +" Trip Id: " +stopTime.getTrip().getTripId()+" Stop Name: "+stopTime.getStop().getStopName());
			}
		} 
	...
	public void startBus(int busId) {
		...
		for (List<StopTime> stopTimes : stopTimesOfTrips) {
			System.out.println(bus.getRoute().getRouteShortName()+" OTOBÜSÜ İÇİN YENİ TRİP:" +  stopTimes.get(0).getTrip().getTripId());
			for (StopTime stopTime : stopTimes) {
				System.out.println("Stop Time Id: "+ stopTime.getStopTimeId()+" Arrival Time: " + stopTime.getArrivalTime()+" Departure Time: " +stopTime.getDepartureTime() +" Trip Id: " +stopTime.getTrip().getTripId()+" Stop Name: "+stopTime.getStop().getStopName());
			}
